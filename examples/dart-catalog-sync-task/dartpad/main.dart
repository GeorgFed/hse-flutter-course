...
bool _throws(dynamic fn, bool Function(Object) accepts) {
  try { (fn as Function)(); } catch (e) { return accepts(e); }
  return false;
}
final throwsUnsupportedError = _Matcher((x) => _throws(x, (e) => e is UnsupportedError), 'UnsupportedError');
final throwsFormatException = _Matcher((x) => _throws(x, (e) => e is FormatException), 'FormatException');
_Matcher contains(String part) => _Matcher((x) => x is String && x.contains(part), 'contains $part');
void expect(dynamic actual, dynamic expected) {
  final ok = expected is _Matcher
      ? expected.matches(actual)
      : const DeepCollectionEquality().equals(actual, expected);
  if (!ok) throw StateError('Ожидалось: ${expected is _Matcher ? expected.label : expected}; получено: $actual');
}
void test(String title, void Function() body) {
  try { body(); _passed++; print('PASS: $title'); }
  catch (e) { _failed++; print('FAIL: $title\n  $e'); }
}
void main() {
  _passed = 0; _failed = 0;
  runChecks();
  print('CHECKS: $_passed/16 passed; $_failed failed');
